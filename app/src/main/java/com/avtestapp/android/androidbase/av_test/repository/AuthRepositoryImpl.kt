...
class AuthRepositoryImpl @Inject constructor(
    ...
    override suspend fun getScore(bearerAndToken: String): Result<Float> {
        return withContext(Dispatchers.IO) {
            try {
                when (val r = getAPIResult(
                    authApiService.getScore(
                        bearerAndToken
                    )
                )) {
                    is Result.Success -> {
                        if (r.result.success) {
                            var result: Float = if (r.result.data == null) {
                                0f
                            }else {
                                r.result.data
                            }
    ...
    override suspend fun submitScore(
        ...
        return withContext(Dispatchers.IO) {
            try {
                when (val r = getAPIResult(
                    authApiService.postScores(
                        bearerAndToken,
                        postUsersScoreRequest
                    )
                )) {
                    is Result.Success -> {
                        if (r.result.success) {
